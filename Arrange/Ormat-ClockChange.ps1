## This script will output the current time for each server upon DTS changes
...
foreach ($domain in $domains) {
  $DCs += (Get-ADDomainController -DomainName $domain -Discover).HostName
}

foreach ($DC in $DCs) {
  $Servers += Get-ADComputer -Filter * -Server $DC -Properties * |Where-Object {$_.OperatingSystem -like "Windows Server*"}
}

foreach($server in $servers){
  if (Test-Connection $server.DNSHostName -Count 1 -ErrorAction SilentlyContinue) {
    $out += Get-DSTInfo -ComputerName $server.DNSHostName -ErrorAction SilentlyContinue |select Computer,StandardChgDate,DaylightChgDate,CurrentTime
  }
  else {
    write-host "The Server "$server.DNSHostName " is unavailable"
  }
}
