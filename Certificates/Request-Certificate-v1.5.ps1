...
.COPYRIGHT (c) 2019 Jonas Feller. All rights reserved.

.TAGS Certificate ActiveDirectory ActiveDirectoryCertificateServices ADCS
...
Thanks to David Allsopp c/o dra27 on GitHub for the contributions!
...
Changes :
    Thanks to Asaf Dori btasaf on GitHub
    - changed to function
    - Added password to switches as default
    - Added "DNS=" to attributes by default
...
'@
        ...
        try    {
            ...
            Invoke-Expression -Command "C:\Windows\System32\certreq.exe -new `"$inf`" `"$req`""
            ...
            Invoke-Expression -Command "C:\Windows\System32\certreq.exe -submit$CAName `"$req`" `"$cer`""
            ...
            Invoke-Expression -Command "C:\Windows\System32\certreq.exe -accept `"$cer`""
            ...
            if ($export) {
                ...
                write-host "##########"(New-Object System.Security.Cryptography.X509Certificates.X509Certificate2((Get-Item $cer).FullName, "")).Thumbprint
                ...
                ##
                $certbytes = $cert.export([System.Security.Cryptography.X509Certificates.X509ContentType]::pfx,$pass)
                #write pfx file
    ...
    #TestPath and create directory
    if(-not(Test-Path "$env:path")){
          $p = ($env:path).split("\")
          New-Item -path "\\il-bck-med2\Lib\Applications\Certificates\" -Name ($p |select -last 1) -ItemType Directory
          }
          
    
    set-location C:\Windows\System32
    $San = $env:SubjectAlternateNames.split()
    $C = "IL"
    $City = "Yavne"
    $Org = "Ormat Technologies"
    $Dep = "IT"
    $tmplName = $env:CertTemplate
    
    New-Certificate -Country $C -State $C -Organisation $Org -City $City -Department $Dep `
    -CN $env:CN -CAName $env:CAname -SAN $San -TemplateName $tmplName -Export -ExportPath $env:path -Password "P@ssw0rd"